...
const TRAM_COLORS = {
  '1':'#5e646a', '2':'#fde500', '3':'#0086cd', '4':'#009d3a', '5':'#e30613',
  '6':'#f59331', '7':'#a4459a', '8':'#ec0d8b', '9':'#76b82a', '10':'#0094d2',
  '11':'#000000','13':'#a87532',
};
...
async function init(){
  ...
  try {
    ...
    renderEmpty('Waiting for the first snapshot to land. The GitHub Action runs every 10 minutes — refresh in a moment.');
    return;
  }
...
function renderKpisFromLatest(){
  ...
  $('kpi-ontime').querySelector('.val').textContent = ontime != null ? ontime.toFixed(1)+'%' : '—';
  // disruptions
  ...
  $('kpi-disruptions').querySelector('.delta').textContent = `${major} major · ${minor} other`;
...
function stripCity(n){ return (n ?? '').replace(/, *Göteborg/i, ''); }
...
async function refreshActiveBoard(){
  ...
  try {
    ...
    console.warn('live worker fetch failed; falling back to latest snapshot', e);
  ...
  for (const d of departures){
    ...
    const estCls = d.cancelled ? '' : (delay > 5 ? 'very-late' : (delay > 2 ? 'late' : ''));
...
function normalizeDeparture(d){
  ...
  if (d.lineName != null || d.delayMin != null || d.estimatedTime !== undefined) return d;
  ...
  const delay = planned && estimated ? Math.round((+new Date(estimated) - +new Date(planned))/60000) : 0;
  return {
...
function renderDisruptions(sits){
  ...
  list.innerHTML = '';
  $('situations-meta').textContent = sits.length ? `${sits.length} active` : 'none active';
  if (sits.length === 0){
    list.innerHTML = `<div class="empty">No active disruptions right now. 🟢</div>`;
    return;
  }
  // Sort newest first
  sits.sort((a,b)=> +new Date(b.creationTime ?? b.startTime ?? 0) - +new Date(a.creationTime ?? a.startTime ?? 0));
...
async function loadPunctuality(){
  let data;
  try { data = await getJSON('data/rollups/punctuality-7d.json?t='+Date.now()); }
  catch { return; }
  ...
  new Chart($('punctuality-chart'), {
    ...
    options:{
      ...
      scales:{y:{min:75,max:100,ticks:{callback:v=>v+'%'},grid:{color:'#1a2342'}},x:{grid:{display:false}}}
    }
  });
}

async function loadDisruptionsByLine(){
  let data; try { data = await getJSON('data/rollups/disruptions-30d.json?t='+Date.now()); } catch { return; }
  ...
  if (top[0]){
    ...
    $('kpi-worst').querySelector('.delta').textContent = `${top[0].totalMinutes} min cumul (30d)`;
  ...
  new Chart($('disrupt-chart'), {
    ...
    options:{
      ...
      scales:{x:{ticks:{callback:v=>v+'m'},grid:{color:'#1a2342'}},y:{grid:{display:false}}}
    }
  });
}

async function loadTrend(){
  let data; try { data = await getJSON('data/rollups/trend-30d.json?t='+Date.now()); } catch { return; }
  const days = data.days ?? [];
  new Chart($('trend-chart'), {
    type:'line',
    data:{
      labels:days.map(d => d.date.slice(5)),
      datasets:[{data:days.map(d=>d.onTimePct),
        borderColor:'#3b9eff', backgroundColor:'rgba(59,158,255,0.12)',
        fill:true, tension:0.35, pointRadius:0, borderWidth:2}]
    },
    options:{
      responsive:true,maintainAspectRatio:false,
      plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>(c.parsed.y??0).toFixed(1)+'%'}}},
      scales:{y:{min:70,max:100,ticks:{callback:v=>v+'%'},grid:{color:'#1a2342'}},
...
async function loadHeatmap(){
  let data; try { data = await getJSON('data/rollups/heatmap-14d.json?t='+Date.now()); } catch { return; }
  const hm = $('heatmap');
  hm.innerHTML = '';
  const lines = Object.keys(data.lines ?? {}).sort((a,b)=>numericOrder(a,b));
  // header row
  hm.appendChild(Object.assign(document.createElement('div'),{className:'row-lab'}));
  for (let h=0; h<24; h++){
    const c = document.createElement('div'); c.className='hour-lab';
  ...
  for (const l of lines){
    ...
    for (let h=0;h<24;h++){
      ...
      cell.title = v == null ? `L${l} · kl ${h}:00 · n/a` : `L${l} · kl ${h}:00 · +${v.toFixed(1)} min`;
      hm.appendChild(cell);
    }
  }
}
...
async function loadTrivia(){
  let data; try { data = await getJSON('data/rollups/trivia-latest.json?t='+Date.now()); } catch { return; }
  if (data.mostPunctualLine){
    const l = data.mostPunctualLine;
    $('trivia-punctual').innerHTML = `Linje ${escapeHTML(l.line)} <span style="color:var(--good)">${(l.onTimePct ?? 0).toFixed(1)}%</span>`;
  ...
  if (data.longestSingleDelay){
    ...
    $('trivia-longest-ctx').textContent = `Line ${x.line ?? '?'} at ${stripCity(x.stop ?? '')} · ${new Date(x.ts).toLocaleString('sv-SE')}`;
  }
  if (data.busiestStop){
    $('trivia-busy').textContent = stripCity(data.busiestStop.name ?? '—');
    $('trivia-busy-ctx').textContent = `${data.busiestStop.count} departures in next hour`;
  }
